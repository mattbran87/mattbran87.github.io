...
**`.gitignore` `vendor/` pattern breaks nested vendor directories:** The `.gitignore` pattern `vendor/` matches ANY directory named `vendor` anywhere in the tree — including `assets/vendor/bootstrap/scss/vendor/` which contains `_rfs.scss`. The negation `!assets/vendor/` only un-ignores the top-level `assets/vendor/` directory, not nested vendor directories within it. This caused `_rfs.scss` to be untracked by git and absent on CI after checkout. **Resolution:** Changed `vendor/` to `/vendor/` (root-only match) so it only ignores the root-level Bundler vendor directory. The `!assets/vendor/` exception was removed since it's no longer needed. **Rule: use leading `/` on `.gitignore` patterns when only the root-level directory should be excluded.**

**Jekyll `exclude` vs Sass `load_paths` on CI (Linux):** An earlier attempt to fix the CI failure added `assets/vendor/` to `_config.yml` `exclude`. This also prevented the Sass compiler from resolving Bootstrap's internal imports on the Linux CI runner, even though `load_paths` pointed there. Removed — the real root cause was the `.gitignore` pattern above. **Rule: never exclude a directory that is also a Sass `load_path` or contains files imported by a `load_path` entry.**
